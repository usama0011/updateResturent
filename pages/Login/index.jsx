
import { LockClosedIcon, MailIcon, MapIcon } from "@heroicons/react/solid";
function index() {

  

    return (
      <div
        style={{
          backgroundImage: `url("/images/medium.jpg")`,
          backgroundRepeat: "no-repeat",
          backgroundSize: "cover",
          backgroundPosition: "center",
        }}
        className=" pt-32 pb-20  bg-opacity-70 transition duration-300  w-full h-full "
      >
        <div className="w-full h-full shadow-lg bg-blue-500"></div>
        <div className="container mx-auto px-4 h-full">
          <div className="flex content-center items-center justify-center h-full">
            <div className="w-full lg:w-4/12 px-4">
              <div className="relative flex flex-col min-w-0 break-words w-full mb-6 shadow-xl rounded-lg bg-gray-300 border-0">
                <div className="rounded-t mb-0 px-6 py-6 ">
                  <div className="text-center mb-3  rounded-lg shadow-lg  bg-blue-500 text-white ">
                    <h6 className=" text-white pt-5 text-sm font-bold">
                      Log in with
                    </h6>
                    <span className="flex mt-5 items-center pb-5 h-full justify-center cursor-pointer space-x-2 md:space-x-3">
                      <img
                        className=" h-10"
                        src="https://img.icons8.com/color/48/000000/google-logo.png"
                      />
                      <img
                        className=" h-10"
                        src="https://img.icons8.com/color/48/000000/facebook-new.png"
                      />
                      <img
                        className=" h-10"
                        src="https://img.icons8.com/color/50/000000/twitter.png"
                      />
                      <img
                        className=" h-10"
                        src="https://img.icons8.com/color/48/000000/instagram-new.png"
                      />
                    </span>
                  </div>
                  <div className="btn-wrapper text-center"></div>
                  <hr className="mt-6 border-b-1 border-gray-400" />
                </div>
                <div className="flex-auto px-4 lg:px-10 py-10 pt-0">
                  <div className="text-gray-500 text-center mb-3 font-bold">
                    <small>Or sign in with credentials</small>
                  </div>
                  <form>
                    <div className="relative w-full mb-3">
                      <label
                        className="block uppercase text-gray-700 text-xs font-bold mb-2"
                        htmlFor="grid-password"
                      >
                        Email
                      </label>
                      <div className="flex items-center relative">
                        <input
                          type="email"
                          className="border-0 px-10 py-3 placeholder-gray-400 text-gray-700 bg-white rounded text-sm shadow focus:outline-none focus:ring w-full"
                          placeholder="Email"
                          style={{ transition: "all .15s ease" }}
                        />
                        <MailIcon className="h-5 absolute left-2 text-gray-600" />
                      </div>
                    </div>

                    <div className="relative w-full mb-3">
                      <label
                        className="block uppercase text-gray-700 text-xs font-bold mb-2"
                        htmlFor="grid-password"
                      >
                        password
                      </label>
                      <div className="flex items-center relative">
                        <input
                          type="password"
                          className="border-0 px-10 py-3 placeholder-gray-400 text-gray-700 bg-white rounded text-sm shadow focus:outline-none focus:ring w-full"
                          placeholder="Password"
                          style={{ transition: "all .15s ease" }}
                        />
                        <LockClosedIcon className="h-5 absolute left-2 text-gray-600" />
                      </div>
                    </div>
                    <div>
                      <label className="inline-flex items-center cursor-pointer">
                        <input
                          id="customCheckLogin"
                          type="checkbox"
                          className="form-checkbox border-0 rounded text-gray-800 ml-1 w-5 h-5"
                          style={{ transition: "all .15s ease" }}
                        />
                        <span className="ml-2 text-sm font-semibold text-gray-700">
                          Remember me
                        </span>
                      </label>
                    </div>

                    <div className="text-center mt-6">
                      <button
                        className="bg-gray-900 text-white active:bg-gray-700 text-sm font-bold uppercase px-6 py-3 rounded shadow hover:shadow-lg outline-none focus:outline-none mr-1 mb-1 w-full"
                        type="button"
                        style={{ transition: "all .15s ease" }}
                      >
                        Log In
                      </button>
                    </div>
                  </form>
                </div>
              </div>
              <div className="flex flex-wrap mt-6">
                <div className="w-1/2">
                  <a
                    href="#pablo"
                    onClick={(e) => e.preventDefault()}
                    className="text-white font-bold"
                  >
                    <small>Forgot password?</small>
                  </a>
                </div>
                <div className="w-1/2 text-right">
                  <a
                    href="#pablo"
                    onClick={(e) => e.preventDefault()}
                    className="text-white font-bold"
                  >
                    <small>Create new account</small>
                  </a>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    );
}

export default index
